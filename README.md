...
The optional top-level `applicationDefaults` configuration supplies per-Application-type `runIntervalMs`, partial plugin `settings`, and partial retention settings by required datafeed name. `deviceDefaults` supplies partial plugin `settings` and partial `datastreams` settings by Device plugin type. Resolved plugin settings merge plugin manifest defaults, type defaults, then concrete Device/Application settings. Application intervals resolve from the core default of `600000`, then the Application-type default, then the concrete Application value. Datastreams resolve from core bootstrap values (`maxBufferLength: 10`, `maxBufferAgeMs: 600000`, and `expectedIntervalMs: 60000`), then Device-type defaults, application datafeed defaults, and finally explicit Device Datastream values. Shared application defaults use maximum buffer length/age and minimum expected interval. Device-type Datastream defaults also provision declared streams without an Application mapping. The bootstrap values make a minimal configuration runnable only; replace them with the device reporting interval and required retention before production use. `gracePeriodCoefficient` is an optional positive Datastream setting that delays an empty-buffer `NO_DATA` alarm until at least `expectedIntervalMs * gracePeriodCoefficient` has elapsed in the current Engine session; its default is `2`. A restored active `NO_DATA` condition remains active during that grace period. The Engine validates the complete resulting settings after merging; see `packages/deployment-ug65-example/settings.example.json` for a deployment example.

### Ecobolt2 plugins

The bundled `sxs.ecobolt2` Device plugin accepts a complete raw payload with numeric `statusBits` from `0` through `65535`, `trapTemp` from `0` through `625`, and `losses` from `0` through `255`. Missing, nonnumeric, non-finite, or out-of-range values reject the complete payload and raise the Device `INVALID_PAYLOAD` diagnostic. Set status bits `1`, `2`, or `256` raise `TEMP_SENSOR_ERROR`, `EXTERNAL_TEMP_SENSOR_ERROR`, or `UNCONFIGURED` respectively, set Device `hwError`, and prevent all Datastream updates for that payload. A later healthy payload clears those conditions.

Healthy Ecobolt2 payloads write `failedOpen` (`statusBits & 32768`), `active` (`!(statusBits & 16384)`), `trapTemp`, and `losses` Datastreams. Boolean values are stored as `1` or `0`. The `sxs.ecobolt2-failed-open` Application requires those four datafeeds and has one `windowSizeMs` setting, defaulting to `3600000`. It uses the last value of each required datafeed in that inclusive window. Missing data remains Undefined during the window grace period and then reports `NO_DATA`; its plugin state is `operState: Undefined`, `trapTemp: null`, and `losses: null`. With values, the Application reports `Warning` when `failedOpen` is true, otherwise `Ok`, and reports `Inactive` or `Active` operating state from `active`.
