...
The bundled Ecobolt2 Device plugin uses type `sxs.ecobolt2` and maps complete validated LoRaWAN payloads into `failedOpen`, `active`, `trapTemp`, and `losses` Datastreams. Its status bits `1`, `2`, and `256` create Device-scoped `TEMP_SENSOR_ERROR`, `EXTERNAL_TEMP_SENSOR_ERROR`, and `UNCONFIGURED` conditions and prevent updates from that payload. The `sxs.ecobolt2-failed-open` Application consumes all four streams, uses a `windowSizeMs` setting, and derives its process state from the most recent complete values in that window.

### Application defaults and Datastream onboarding

The optional top-level `applicationDefaults` object is keyed by Application plugin type ID. Its `runIntervalMs` overrides the core ten-minute Application interval when an instance does not set one. Its `settings` object may provide partial plugin settings, and its `datafeedDatastreams` object is keyed by the plugin's required datafeed names and may provide partial Datastream retention settings. The optional `deviceDefaults` object is keyed by Device plugin type ID; its `settings` object may provide partial plugin settings and its `datastreams` object may provide partial settings for the streams declared by that Device plugin. Use the nested `datastreams` object rather than stream names beside `settings`, so the configuration has a stable schema.

Entity plugin settings merge in order: plugin manifest defaults, type defaults, then the concrete Device/Application `settings` object. An Application interval resolves in order from the core default of 600000 milliseconds, Application-type default, then concrete Application value. For every Application datafeed mapping, the engine applies that type's datafeed defaults to the referenced Device Datastream. When several Applications target the same Datastream, their partial defaults aggregate deterministically: use the maximum `maxBufferLength`, maximum `maxBufferAgeMs`, minimum `expectedIntervalMs`, and maximum `gracePeriodCoefficient`. Datastreams resolve in order from core bootstrap defaults (`maxBufferLength: 10`, `maxBufferAgeMs: 600000`, and `expectedIntervalMs: 60000`), Device-type defaults, application datafeed defaults, then explicit Device Datastream values. Device-type defaults provision declared Datastreams even without an Application mapping. The core defaults make minimal configurations runnable but are not production recommendations; a deployment must provide timings and retention appropriate to its devices. The optional positive `gracePeriodCoefficient` delays an empty-buffer `NO_DATA` alarm until no earlier than `expectedIntervalMs * gracePeriodCoefficient` after session start; the default is `2`. The final result is validated after merging.
